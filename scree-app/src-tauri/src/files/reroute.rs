...
//! `lib/drums.scree` beside the file that wrote it. Renaming that file, or
...
use crate::parser::parser::{parse, ScreeItem, Use, UseTree};
...
struct Import {
    ...
    /// is a definition inside `lib/drums.scree` rather than a folder step.
    item: Option<String>,
...
fn imports_in(items: &[ScreeItem], code: &str, path: &Path) -> Vec<Import> {
    let dir = path.parent().unwrap_or(Path::new(""));
    items
        .iter()
        .filter_map(|item| match item {
            ScreeItem::Use(u) => one(u, code, dir),
            _ => None,
        })
        .collect()
}
...
/// `lib/drums/kick.scree` exists and an item of `lib/drums.scree` when it does
...
mod tests {
    ...
    fn a_renamed_module_is_followed() {
        ...
        p.write("drums.scree", "let kick = 1\n");
        p.write("song.scree", "use drums::kick\nplay(kick)\n");

        assert_eq!(p.move_file("drums.scree", "percussion.scree"), vec![]);
        assert_eq!(p.read("song.scree"), "use percussion::kick\nplay(kick)\n");
    }
    ...
    fn an_existing_alias_is_left_to_say_what_it_says() {
        ...
        p.write("drums.scree", "let kick = 1\n");
        p.write("song.scree", "use drums as d\nplay(d::kick)\n");

        assert_eq!(p.move_file("drums.scree", "percussion.scree"), vec![]);
        assert_eq!(p.read("song.scree"), "use percussion as d\nplay(d::kick)\n");
    }
    ...
    fn a_module_moved_deeper_is_reached_through_the_folder() {
        ...
        assert_eq!(p.move_file("drums.scree", "lib/drums.scree"), vec![]);
        assert_eq!(p.read("song.scree"), "use lib::drums::{kick}\nplay(kick)\n");
    }
    ...
    fn a_glob_keeps_its_star() {
        ...
        p.write("lib/drums.scree", "let kick = 1\n");
        p.write("song.scree", "use lib::drums::*\nplay(kick)\n");

        assert_eq!(p.move_file("lib/drums.scree", "drums.scree"), vec![]);
        assert_eq!(p.read("song.scree"), "use drums::*\nplay(kick)\n");
    }
    ...
    fn moving_the_importer_rewrites_its_own_paths() {
        ...
        p.write("lib/drums.scree", "let kick = 1\n");
        p.write("song.scree", "use lib::drums::kick\nplay(kick)\n");

        assert_eq!(p.move_file("song.scree", "lib/song.scree"), vec![]);
        assert_eq!(p.read("lib/song.scree"), "use drums::kick\nplay(kick)\n");
    }
    ...
    fn imports_within_a_moved_folder_are_left_alone() {
        ...
        p.write("lib/drums.scree", "let kick = 1\n");
        p.write("lib/kit.scree", "use drums::kick\nlet k = kick\n");

        assert_eq!(p.move_file("lib", "sounds"), vec![]);
        assert_eq!(p.read("sounds/kit.scree"), "use drums::kick\nlet k = kick\n");
    }
    ...
    fn a_module_moved_out_of_reach_is_reported() {
        ...
        let unfollowed = p.move_file("lib/drums.scree", "drums.scree");
        assert_eq!(unfollowed.len(), 1, "got {unfollowed:?}");
        assert!(unfollowed[0].file.ends_with("kit.scree"));
        assert!(
        ...
        assert_eq!(p.read("lib/kit.scree"), "use drums::kick\nlet k = kick\n");
    }
    ...
    fn a_folder_that_cannot_be_written_as_a_path_is_reported() {
        ...
        let unfollowed = p.move_file("drums.scree", "my drums/drums.scree");
        assert_eq!(unfollowed.len(), 1, "got {unfollowed:?}");
        assert_eq!(p.read("song.scree"), "use drums::kick\nplay(kick)\n");
    }
    ...
    fn unrelated_imports_are_not_rewritten() {
        ...
        p.write("drums.scree", "let kick = 1\n");
        p.write("bass.scree", "let low = 1\n");
        p.write("song.scree", "use bass::low\nuse drums::kick\nplay(kick)\n");
    ...
    fn several_imports_in_one_file_are_all_corrected() {
        ...
        p.write("lib/drums.scree", "let kick = 1\n");
        p.write("lib/bass.scree", "let low = 1\n");
        p.write(
            "song.scree",
            "use lib::drums::kick\nuse lib::bass::low\nplay(kick)\n",
        );
    ...
    fn a_file_that_does_not_parse_is_reported_rather_than_edited() {
        ...
        let unfollowed = p.move_file("drums.scree", "percussion.scree");
        assert_eq!(unfollowed.len(), 1, "got {unfollowed:?}");
