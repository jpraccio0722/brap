...
//! to the file the `use` is written in — `use lib::drums` is `lib/drums.scree`
...
//! are; only its `fn`s and `let`s travel. This is the one place scree differs
//! from what a Rust reader would guess, and it is the same difference Rust
//! makes between an item and a statement.
//!
//! One import is written for you. The project's `patterns.scree` — what the
...
//! `Vec<ScreeItem>`. By the time the lowerer or the scheduler sees a program
...
use crate::parser::parser::{parse, ScreeItem, Use, UseName, UseTree};

/// The extension a module file carries. A `use` path never writes it.
pub const EXTENSION: &str = "scree";
...
pub fn expand(
    ...
    let wants_files = items.iter().any(|i| matches!(i, ScreeItem::Use(_)));
...
impl<'a> Resolver<'a> {
    ...
    fn file(
        ...
        for item in items {
            match item {
                ScreeItem::Use(u) => uses.push(u),
                // A module lends out its definitions, not its performance.
                ScreeItem::Expr(_) | ScreeItem::Call { .. } if prefix.is_some() => {}
...
fn own_names(items: &[ScreeItem], prefix: Option<&str>) -> HashMap<String, String> {
    let mut names = HashMap::new();
    for item in items {
        let own = match item {
            ScreeItem::Function { name, .. } | ScreeItem::Let { name, .. } => &name.0,
            _ => continue,
        };
...
fn define_as(item: &mut ScreeItem, names: &HashMap<String, String>) {
    let name = match item {
        ScreeItem::Function { name, .. } | ScreeItem::Let { name, .. } => name,
        _ => return,
    };
