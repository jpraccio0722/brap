...
use crate::parser::parser::{Expr, ScreeItem, Statement};
...
pub(crate) fn paths_in(items: &[ScreeItem]) -> Vec<String> {
    ...
    for item in items {
        match item {
            ScreeItem::Function { body, .. } => walk(body, &mut found),
            ScreeItem::Let { value, .. } => walk(value, &mut found),
            ScreeItem::Expr(e) => walk(e, &mut found),
            ScreeItem::Call { args, .. } => {
                for arg in args {
...
";
    ...
    fn a_wave_on_disk(name: &str) -> (PathBuf, PathBuf) {
        let dir = std::env::temp_dir().join(format!("scree-samples-{}-{name}", std::process::id()));
