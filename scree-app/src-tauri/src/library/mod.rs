...
//! `kit/kick.scree` beside the file — [`crate::imports::module_file`] says so —
...
//! 2. the project's own copies, in `.scree/libraries/`,
...
//! root/
//!   kit.scree        the module itself, and/or
...
//! is not a `.scree`, so it can never be imported, and it is how a store
...
pub const EXTENSION: &str = "screepack";
...
pub const PROJECT_DIR: &str = ".scree";

/// What a project says it is when it is packed, beside `scree-project.json`
...
/// folder.
pub const PROJECT_MANIFEST: &str = "scree-library.json";
...
///
/// `kit.scree` or anything under `kit/`, and nothing else. This is the check
...
/// folder holding `<name>.scree` and/or `<name>/`.
...
/// are relative to. `kit.scree` sits beside every other library in the store,
...
fn audit(sources: &[PathBuf], name: &str) -> Result<(), String> {
    ...
    for source in sources {
        ...
        for file in modules_under(source) {
            ...
            for item in &items {
                let ScreeItem::Use(u) = item else { continue };
...
/// Every `.scree` file at or under a path.
