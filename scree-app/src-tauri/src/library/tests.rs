...
fn a_kit(at: &Path) -> PathBuf {
    pack(
        ...
        KIT,
        &[
            ("root/kit.scree", "use kit::drums::*\nfn hit(f) = kick(f)\n"),
            ("root/kit/drums.scree", "fn kick(f) = sin(f)\n"),
...
fn a_pack_unpacks_into_the_store() {
    ...
    assert!(store.join("kit.scree").is_file());
    assert!(store.join("kit/drums.scree").is_file());
...
fn a_pack_may_not_write_outside_its_own_name() {
    ...
    let greedy = pack(
        ...
        KIT,
        &[
            ("root/kit.scree", "fn hit(f) = sin(f)\n"),
            ("root/drums.scree", "fn kick(f) = sin(f)\n"),
        ],
    );

    let err = install(&greedy, &store, Source::Global, false).expect_err("should refuse");
    assert!(err.contains("drums.scree"), "the message should name it, got {err}");

    // And nothing of it survives the refusal — not even the part that was fine.
    assert!(!store.join("kit.scree").exists());
...
fn a_pack_may_not_climb_out_of_itself() {
    ...
    let escaping = pack(&from, KIT, &[("root/../../elsewhere.scree", "fn x() = 1\n")]);
    ...
    assert!(!from.parent().unwrap_or(&from).join("elsewhere.scree").exists());
...
fn a_file_that_is_not_a_pack_is_named_as_one() {
    ...
    let not_a_pack = from.join("song.screepack");
    ...
    let no_manifest = pack(&from, "", &[("root/kit.scree", "fn hit(f) = sin(f)\n")]);
...
fn a_name_that_is_not_an_identifier_is_refused() {
    ...
    let bad = pack(
        ...
        &[("root/909 kit.scree", "fn hit(f) = sin(f)\n")],
    );
...
fn a_second_install_of_one_name_asks_before_replacing() {
    ...
    assert_eq!(
        std::fs::read_to_string(store.join("kit.scree")).expect("should read"),
        "use kit::drums::*\nfn hit(f) = kick(f)\n"
    );
...
fn a_packed_folder_installs_as_it_was() {
    ...
    std::fs::write(source.join("kit.scree"), "use kit::drums::*\n").expect("should write");
    std::fs::write(source.join("kit/drums.scree"), "fn kick(f) = sin(f)\n").expect("should write");
    ...
    assert_eq!(
        std::fs::read_to_string(store.join("kit/drums.scree")).expect("should read"),
        "fn kick(f) = sin(f)\n"
    );
    assert!(store.join("kit/samples/909.wav").is_file());
...
fn exporting_a_name_the_folder_does_not_hold_is_refused() {
    ...
    std::fs::write(source.join("song.scree"), "sin(440)\n").expect("should write");
    ...
    assert!(err.contains("kit.scree"), "unexpected message: {err}");
...
fn an_outcome_serializes_to_the_shape_the_editor_expects() {
    ...
    let newer = pack(&from, r#"{ "name": "kit", "version": "2.0.0" }"#, &[("root/kit.scree", "")]);
...
fn an_absolute_sample_path_will_not_pack() {
    let err = export_of(
        &[("kit/hit.scree", "fn hit(f) = sample(load(\"/Users/me/909.wav\"), ramp())\n")],
        "audit-absolute",
    )
...
fn a_sample_path_out_of_the_library_will_not_pack() {
    let err = export_of(
        &[("kit/hit.scree", "fn hit(f) = sample(load(\"../breaks/amen.wav\"), ramp())\n")],
        "audit-escape",
    )
    .expect_err("should refuse");
...
fn a_library_that_names_only_its_own_files_packs() {
    export_of(
        &[
            ("kit.scree", "use kit::drums::*\nfn hit(f) = kick(f)\n"),
            ("kit/drums.scree", "fn kick(f) = sample(load(\"samples/909.wav\"), ramp())\n"),
...
fn the_top_module_may_name_its_own_folder() {
    export_of(
        &[
            ("kit.scree", "fn hit(f) = sample(load(\"kit/samples/909.wav\"), ramp())\n"),
            ("kit/samples/909.wav", "not really audio"),
        ],
        "audit-own-folder",
    )
...
fn a_library_can_be_packed_installed_and_played() {
    ...
    std::fs::create_dir_all(source.join("kit/samples")).expect("should make");
    std::fs::write(
        source.join("kit.scree"),
        "use kit::drums::*\nfn hit(f, decay = 0.25) = kick(f) * perc(0.001, decay)\n",
    )
    .expect("should write");
    std::fs::write(
        source.join("kit/drums.scree"),
    ...
    // library by name.
    let song = project.join("song.scree");
