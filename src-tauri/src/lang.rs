...
pub static RANDOM_BUILTINS: &[ListBuiltin] = &[
    ...
    ListBuiltin {
        ...
        receives: ValueKind::List,
        returns: ValueKind::List,
        doc: "`count` elements drawn without replacement, in a random order — no element twice, unlike `count` calls to `choice`. Asking for the whole list is `scramble`; asking for more than it holds is an error.",
    },
    ListBuiltin {
        name: "randscale",
