...
/// Expand `entry` against `modules`, as if it were being run from `/p/song.scree`.
fn expand_ok(entry: &str, modules: &[(&str, &str)]) -> Vec<ScreeItem> {
...
fn defined(items: &[ScreeItem]) -> Vec<String> {
    items
        .iter()
        .filter_map(|i| match i {
            ScreeItem::Function { name, .. } | ScreeItem::Let { name, .. } => Some(name.0.clone()),
            _ => None,
        })
        .collect()
}
...
fn constant_of(items: &[ScreeItem]) -> f64 {
...
/// A folder deep: `use lib::drums` is `lib/drums.scree`, and the module's name
/// is the path it was written as.
#[test]
fn a_path_walks_into_folders() {
    let items = expand_ok(
        "use lib::drums\nplay([50], drums::kick)\n",
        &[("/p/lib/drums.scree", DRUMS)],
    );
...
fn private_names_do_not_collide() {
    let items = expand_ok(
        "use a::*\nsin(one())\n",
        &[
            ("/p/a.scree", "fn helper() = 1\nfn one() = helper()\n"),
            ("/p/b.scree", "fn helper() = 2\nfn two() = helper()\n"),
        ],
    );
    assert_eq!(
        constant("use a::*\nuse b::*\nsin(one())\n", &[
...
fn imports_are_transitive() {
    let items = expand_ok(
        "use leads::*\nplay([50], lead)\n",
        &[
            ("/p/leads.scree", "use tones::*\nfn lead(f) = tone(f) * 0.5\n"),
            ("/p/tones.scree", "fn tone(f) = sin(f)\n"),
        ],
    );
...
fn a_missing_module_says_where_it_looked() {
    ...
    assert!(
        err.message.contains("/p/drums.scree"),
...
fn a_cycle_is_reported_as_the_route_that_closed_it() {
    ...
    assert!(err.message.contains("a.scree"), "got: {}", err.message);
}
...
fn a_broken_module_reports_its_own_position() {
    ...
    assert_eq!(err.file.as_deref(), Some("/p/m.scree"));
}
...
fn the_editors_payload_deserializes() {
    let sent = serde_json::json!({ "path": "/p/song.scree", "root": "/p" });
    let ws: Workspace = serde_json::from_value(sent).expect("should deserialize");
    assert_eq!(ws.dir(), Some(PathBuf::from("/p")));
    assert_eq!(ws.patterns_path(), PathBuf::from("/p/patterns.scree"));
...
fn expand_in(src: &str, ws: &Workspace) -> Vec<ScreeItem> {
...
fn the_projects_patterns_need_no_use() {
    let project = Project::new("needs-no-use", &[("patterns.scree", PATTERNS)]);
    let items = expand_in(PLAYS_HATS, &project.running("song.scree"));
...
fn a_drawn_pattern_can_be_qualified() {
    let project = Project::new("qualified", &[("patterns.scree", PATTERNS)]);
    let items = expand_in(
        "fn hat() = noise()\nplay(patterns::hats, hat)\n",
        &project.running("song.scree"),
    );
    assert!(lower(&items).is_ok());
}
...
fn a_file_may_shadow_a_drawn_pattern() {
    let project = Project::new("shadow", &[("patterns.scree", PATTERNS)]);
    let items = expand_in(
        "fn hat() = noise()\nlet hats = [\\, \\]\nplay(hats, hat)\n",
        &project.running("song.scree"),
    );
...
fn patterns_reach_the_whole_project() {
    let project = Project::new("subfolder", &[("patterns.scree", PATTERNS)]);
    let items = expand_in(PLAYS_HATS, &project.running("parts/verse.scree"));
    assert!(lower(&items).is_ok());
}
...
fn a_module_does_not_get_the_patterns() {
    let project = Project::new(
        "module-patterns",
        &[
            ("patterns.scree", PATTERNS),
            ("m.scree", "fn beat() = hats\n"),
        ],
    );
...
fn running_the_patterns_file_is_not_a_cycle() {
    ...
    let project = Project::new("self", &[("patterns.scree", &src)]);
    let items = parse(src.clone()).expect("should parse");
    assert!(expand(items, &src, &project.running("patterns.scree")).is_ok());
}
...
fn no_patterns_file_changes_nothing() {
    ...
    let items = parse(src.to_string()).expect("should parse");
    assert_eq!(
        expand(items.clone(), src, &project.running("song.scree")).expect("should expand"),
        items
    );
}
...
fn the_patterns_file_plays_on_its_own() {
    let project = Project::new("on-disk", &[("patterns.scree", PATTERNS)]);
    let items = expand_in(PLAYS_HATS, &project.running("song.scree"));
...
fn the_panels_rows_are_the_patterns_file() {
    let project = Project::new("panel-wins", &[("patterns.scree", "let hats = [\\, \\, \\]\n")]);
    let mut ws = project.running("song.scree");
...
fn an_emptied_panel_empties_the_patterns() {
    let project = Project::new("emptied", &[("patterns.scree", PATTERNS)]);
    let mut ws = project.running("song.scree");
...
fn only_the_patterns_file_comes_from_the_editor() {
    ...
    assert_eq!(
        ws.read(&project.dir.join("m.scree")).as_deref(),
        Some("fn v() = 1\n"),
...
fn a_module_is_read_from_the_disk_every_time() {
    let project = Project::new("resaved", &[("m.scree", "fn v() = 1\n")]);
    let ws = project.running("song.scree");
    let src = "use m::*\nsin(v())\n";

    let first = expand_in(src, &ws);
    assert_eq!(constant_of(&first), 1.0);

    std::fs::write(project.dir.join("m.scree"), "fn v() = 2\n").expect("should rewrite");
...
fn modules_are_read_from_the_disk() {
    ...
    std::fs::write(dir.join("lib").join("drums.scree"), DRUMS).expect("should write");

    let song = dir.join("song.scree");
...
fn the_example_project_compiles() {
    ...
    let song = examples.join("imports.scree");
    let src = std::fs::read_to_string(&song).expect("examples/imports.scree should exist");

    let ws = Workspace::new(Some(song.display().to_string()), None);
    let items = expand(parse(src.clone()).expect("should parse"), &src, &ws)
        .unwrap_or_else(|e| panic!("examples/imports.scree should expand: {e}"));
...
fn no_use_survives_expansion() {
    ...
    assert!(!items.iter().any(|i| matches!(i, ScreeItem::Use(_))));
}
